...
Scans a directory for files with version numbers in their names
(e.g., "request_config 0.1.5.1.json"), groups them by base name,
and creates sequential git commits — one per version — with the clean
filename (e.g., "request_config.json") and version in the commit message.
...
Examples:
    ...
    python git_versioner.py ./output/page_123 /path/to/repo --dry-run

    # Initialize a new repo and commit versions:
    python git_versioner.py ./output/page_123 /path/to/repo --init

    # Commit to an existing repo:
    python git_versioner.py ./output/page_123 /path/to/repo
"""
...
def find_versioned_files(source_dir: Path) -> dict[tuple[str, str], list[tuple[str, Path]]]:
    """Find files with version patterns and group by (base_name, extension).

    Returns:
        {("request_config", "json"): [("0.1.5.1", Path), ("0.1.5.2", Path), ...]}
    ...
    for entry in sorted(source_dir.iterdir()):
        ...
        if match:
            ...
            groups[(name, ext)].append((version, entry))
...
def commit_versions(source_dir: Path, target_repo: Path, *, dry_run: bool = False) -> int:
    """Process versioned files and create one git commit per version.
    ...
    for (name, ext), versions in sorted(groups.items()):
        target_name = f'{name}.{ext}'
        logging.info('--- %s: %d versions ---', target_name, len(versions))

        for version_str, source_path in versions:
            commit_msg = f'{target_name} version {version_str}'
            ...
            shutil.copy2(source_path, target_repo / target_name)
            git('add', target_name, cwd=target_repo)
...
def main():
    ...
    parser.add_argument('source_dir',
                        help='Directory containing versioned files')
